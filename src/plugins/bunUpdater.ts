...
async function getBunVersion(): Promise<string> {
    try {
        ...
        log.error(`获取Bun版本失败: ${err.message}`);
        return '未知';
    }
}
...
async function updateBunAndRestart(): Promise<string> {
    try {
        ...
        log.info(`开始更新Bun，当前版本: ${oldVersion}`, { remote: true });
        ...
        log.info(`Bun已更新: ${oldVersion} -> ${newVersion}，即将重启机器人...`, { remote: true });
        ...
        setTimeout(() => {
            ...
            try {
                ...
                // 记录启动失败的错误但仍然退出
                log.error(`启动新进程失败: ${spawnError}`, { remote: true });
            }
        ...
        const err = error instanceof Error ? error : new Error(String(error));
        log.error(`更新Bun失败: ${err.message}`, { remote: true });
