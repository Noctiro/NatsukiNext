...
async function startAiGame(ctx: CommandContext) {
    ...
    try {
        ...
        await ctx.message.replyMedia(
            ...
            {
                caption: html`红方（您）VS ${difficultyText}AI<br>请输入您的走法，例如：/chess move 炮二平五`
            }
        );
    } catch (error) {
...
async function acceptChallenge(ctx: CommandContext) {
    ...
    try {
        ...
        await ctx.message.replyMedia(
            ...
            {
                caption: html`游戏开始！红方先行，请输入您的走法，例如：/chess move 炮二平五`
            }
        );
    } catch (error) {
        // 如果图片渲染失败，fallback到HTML渲染
...
async function updateGameBoard(game: Game, ctx: CommandContext) {
    ...
    try {
        ...
        await ctx.message.replyMedia(
            ...
            {
                caption: html`轮到${html(currentPlayer)}行动${ game.lastMove ? `，上一步：${game.lastMove}` : '' }`
            }
        );
    } catch (error) {
        // 如果图片渲染失败，fallback到HTML渲染
...
async function processAIMove(game: Game, ctx: CommandContext) {
    ...
    try {
        ...
        await ctx.message.replyMedia(
            ...
            {
                caption: html`${statusMessage}${ game.lastMove ? `，AI走法：${game.lastMove}` : '' }`
            }
        );
    } catch (error) {
        // 如果图片渲染失败，fallback到HTML渲染
