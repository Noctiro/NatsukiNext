...
import { createCanvas, GlobalFonts } from '@napi-rs/canvas'; // Removed registerFont, CanvasRenderingContext2D, added GlobalFonts
...
export class BoardRenderer {
        ...
        if (!this.fontLoaded) {
            try {
                ...
                if (fs.existsSync(fontPath)) {
                    ...
                    this.logger?.info('使用项目内置 MiSans-Regular 字体');
                    ...
                    this.logger?.info('使用系统 SimSun 字体');
                    ...
                    this.logger?.info('使用系统 Arial Unicode 字体');
                } else {
                    this.logger?.warn('未找到适合的中文字体，将使用系统默认字体');
                    // Fallback font family might be needed here depending on the system
                }

                this.fontLoaded = true;
            } catch (err) {
                this.logger?.error('字体加载错误:', err);
    ...
    static async drawBoardImage(game: Game, showCoordinates: boolean = true): Promise<Buffer> {
        ...
        try {
            ...
            this.logger?.error('创建图片缓冲区失败:', error);
            throw error;
        }
    }
